...
class AmqpProtocol(AMQClient):
    ...
    def connectionClose(self, reason):
        #print('AmqpProtocol::connectionClose') 
        self.connected = False 
        self.close(reason)
...
class AmqpFactory(protocol.ReconnectingClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        if (self.p != None):
            ##print('residue protocol wiping')
            self.p.connectionClose('client hangup')
            self.stopTrying()
            self.stopFactory()
            reactor.callLater(1,self.p.transport.loseConnection)
            self.p = None
            self.client = None
            self = None # hehe ;o)
